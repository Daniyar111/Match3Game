...
import 'package:match3_game/src/animations/animation_chain.dart';
import 'package:match3_game/src/animations/animation_combo_collapse.dart';
import 'package:match3_game/src/animations/animation_combo_three.dart';
import 'package:match3_game/src/animations/animation_swap_tiles.dart';
import 'package:match3_game/src/data/blocs/game_bloc.dart';
import 'package:match3_game/src/data/providers/bloc_provider.dart';
import 'package:match3_game/src/models/animation_sequence.dart';
import 'package:match3_game/src/models/audio_type.dart';
import 'package:match3_game/src/models/combo.dart';
import 'package:match3_game/src/models/combo_type.dart';
import 'package:match3_game/src/models/level.dart';
import 'package:match3_game/src/models/row_col.dart';
import 'package:match3_game/src/models/tile.dart';
import 'package:match3_game/src/models/tile_type.dart';
import 'package:match3_game/src/utils/animations_resolver.dart';
import 'package:match3_game/src/utils/array_2d.dart';
import 'package:match3_game/src/utils/audio.dart';
import 'package:match3_game/src/widgets/board.dart';
import 'package:match3_game/src/widgets/game_moves_left_panel.dart';
import 'package:match3_game/src/widgets/game_over_splash.dart';
import 'package:match3_game/src/widgets/game_splash.dart';
import 'package:match3_game/src/widgets/objective_panel.dart';
import 'package:match3_game/src/widgets/shadowed_text.dart';
...
class _GamePageState extends State<GamePage> with SingleTickerProviderStateMixin {
  ...
  RowCol _rowColFromGlobalPosition(Offset globalPosition) {
    ...
    return RowCol(
      ...
      row: widget.level.numberOfRows - (top / widget.level.tileHeight).floor() - 1,
    );
  }
  ...
  void _onPanDown(DragDownDetails details) {
    ...
    if (rowCol.row < 0 || rowCol.row >= widget.level.numberOfRows || rowCol.col < 0 || rowCol.col >= widget.level.numberOfCols) {
      return;
    }
  ...
  void _onPanUpdate(DragUpdateDetails details) {
    ...
    if (gestureStarted) {
      ...
      if (delta.dx.abs() > delta.dy.abs() && delta.dx.abs() > _MIN_GESTURE_DELTA) {
      ...
      if (test == true) {
        ...
        if (rowCol.col < 0 || rowCol.col == widget.level.numberOfCols || rowCol.row < 0 || rowCol.row == widget.level.numberOfRows) {
          ...
          if (canBePlayed) {
            ...
            bool swapAllowed = gameBloc.gameController.swapContains(gestureFromTile, destTile);
            ...
            gameBloc.gameController.grid[gestureFromRowCol.row][gestureFromRowCol.col].visible = false;
                ...
                builder: (BuildContext context) {
                  ...
                  return AnimationSwapTiles(
                    ...
                    onComplete: () async {
                      ...
                      gameBloc.gameController.grid[rowCol.row][rowCol.col].visible = true;

                      gameBloc.gameController.grid[gestureFromRowCol.row][gestureFromRowCol.col].visible = true;
                      ...
                      if (swapAllowed == true) {
                        ...
                        Combo comboOne = gameBloc.gameController.getCombo(gestureFromTile.row, gestureFromTile.col);
                        Combo comboTwo = gameBloc.gameController.getCombo(destTile.row, destTile.col);

                        // Wait for both animations to complete
                        await Future.wait([_animateCombo(comboOne), _animateCombo(comboTwo)]);

                        // Resolve the combos
                        gameBloc.gameController.resolveCombo(comboOne, gameBloc);
                        gameBloc.gameController.resolveCombo(comboTwo, gameBloc);
  ...
  Future<dynamic> _playAllAnimations() async {
    ...
    //
    AnimationsResolver animationResolver = AnimationsResolver(gameBloc: gameBloc, level: widget.level);
    ...
    List<AnimationSequence> sequences = animationResolver.getAnimationsSequences();
